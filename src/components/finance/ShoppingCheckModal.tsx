...
interface ShoppingCheckModalProps {
  event: FinanceEvent & { date: string };
  onClose: () => void;
  onSuccess: () => void;
}

export function ShoppingCheckModal({ event, onClose, onSuccess }: ShoppingCheckModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        <div className="flex flex-col items-center text-center mb-8">
          ...
          <h2 className="text-2xl font-black mb-1">Как закупились?</h2>
          <p className="text-zinc-500 font-medium">Введите итоговую сумму чека для закупки «{event.name}»</p>
        </div>
